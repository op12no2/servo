...
- **Moves stop short of goal (not yet fixed):** servos settle 1–6 steps short of the goal, on the side they approached from (below → under, above → over), well outside the dead zone of 1. Likely cause: PID is P = 15 (0x15), D = 15 (0x16), **I = 0** (0x17), minimum starting force 45 = 4.5% (0x18, u16). With no integral term, drive near the goal (~P × error) drops below gear friction and the servo stalls. Untested fixes (EPROM writes, so unlock/lock): a small I (e.g. 2), or a higher minimum starting force (e.g. ~80). Separately, `verify_move` sometimes reports "stopped" too early: the moving flag can read 0 for 100 ms and the servo then creeps on (reported 475, read 482 a second later).
