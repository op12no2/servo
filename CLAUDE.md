...
- **Stopping short and settling:** the earlier 1–6-step misses (short on the approach side) happened while the servo had tipped over with its axle rubbing on the desk. Unloaded, 12 back-and-forth moves at time 500 all settled within ±2 (mostly ±1) with no creep a second later. Settings are unchanged: P = 15 (0x15), D = 15 (0x16), **I = 0** (0x17), minimum starting force 45 = 4.5% (0x18, u16). If it stops short under real load, try (EPROM writes, so unlock/lock) a small I (e.g. 2) or a higher minimum starting force (e.g. ~80). Unloaded, about 1 move in 10 still stops ±2 (occasionally ±3) off, so a dead zone of 1 gets the odd report. On fast moves (time 0–300) the servo overshoots by 4–5 steps, the moving flag drops to 0 at the peak, and it eases back with the flag still 0, sometimes after a pause of 100 ms or more. The moving flag can also read 0 mid-travel. `verify_move` handles this (see Architecture); over 80 test moves no off-goal report was followed by creep, only 1-step drifts inside the dead zone.
...
- **Register helpers:** `read_regs` / `write_regs` / `read_u16` / `write_u16`. 16-bit registers are high byte first; convert through `get16()` / `put16()` rather than open-coding the byte order. They don't print status-reply error bits (overload etc.); they OR them into `status_err[id]`. A command appends `status_tail(id)` to its output line for that id, and `dispatch()` calls `flush_status()` after each `run()` to print anything left, so each id's status shows once per command, on its own line.
